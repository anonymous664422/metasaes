...
@st.cache_resource
def load_tokenizer_and_dataset(dataset=None, context_size=128, prepend_bos=True):
...
def meta_feature_button_callback(meta_feature_idx):
    st.query_params.from_dict({"page": "Meta Feature Explorer", "meta_feature": meta_feature_idx})
...
def feature_button_callback(feature_idx):
    st.query_params.from_dict({"page": "Feature Explorer", "feature": feature_idx})
...
def main():
    ...
    page = query_params.get("page", "Feature Explorer")
    feature_idx = int(query_params.get("feature", 0))
    meta_feature_idx = int(query_params.get("meta_feature", 0))
    ...
    if st.session_state.page == "Feature Explorer":
        ...
        with st.spinner("Generating graph..."):
            ...
            #                     st.query_params.from_dict({"page": "Feature Explorer", "feature": f_idx})
            #                     st.rerun()
